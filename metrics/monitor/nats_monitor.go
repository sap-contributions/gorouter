...
func (n *NATSMonitor) Run(signals <-chan os.Signal, ready chan<- struct{}) error {
	...
	for {
		...
		case <-n.TickChan:
			...
			if err != nil {
				n.Logger.Error("error-retrieving-nats-subscription-pending-messages", goRouterLogger.ErrAttr(err))
			}
			chainer := n.Sender.Value("buffered_messages", float64(queuedMsgs), "message")
			err = chainer.Send()
			if err != nil {
				n.Logger.Error("error-sending-buffered-messages-metric", goRouterLogger.ErrAttr(err))
			}

			droppedMsgs, err := n.Subscriber.Dropped()
			if err != nil {
				n.Logger.Error("error-retrieving-nats-subscription-dropped-messages", goRouterLogger.ErrAttr(err))
			}
			chainer = n.Sender.Value("total_dropped_messages", float64(droppedMsgs), "message")
			err = chainer.Send()
			if err != nil {
				n.Logger.Error("error-sending-total-dropped-messages-metric", goRouterLogger.ErrAttr(err))
			}
		case <-signals:
